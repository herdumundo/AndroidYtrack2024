package com.portalgmpy.y_trackcomercial.services.websocket
...
class PieSocketListener @Inject constructor(
    ...
    private val countOinvPendientesUseCaseAlarmManager: CountOinvPendientesUseCaseAlarmManager,
    private val getOinvPendientesExportarUseCaseAlarmManager: GetOinvPendientesExportarUseCaseAlarmManager,
    private val exportarOinvPendientesUseCaseAlarmManager: ExportarOinvPendientesUseCaseAlarmManager,

    private val countNroFacturaPendienteUseCaseAlarmManager: CountNroFacturaPendienteUseCaseAlarmManager,
    private val getNroFacturaPendienteUseCaseAlarmManager: GetNroFacturaPendienteUseCaseAlarmManager,
    private val exportarNroFacturaPendientesUseCaseAlarmManager: ExportarNroFacturaPendientesUseCaseAlarmManager,

    private val countOinvNoProcesadasSapUseCaseAlarmManager: CountOinvNoProcesadasSapUseCaseAlarmManager,
    private val importarFacturasProcesadasSapUseCaseAlarmManager: ImportarFacturasProcesadasSapUseCaseAlarmManager,

    private val getVisitasPendientesUseCase: GetVisitasPendientesUseCaseAlarmManager,
    private val enviarVisitasPendientesUseCase: EnviarVisitasPendientesUseCaseAlarmManager,
    private val countCantidadPendientes: CountCantidadPendientesAlarmManager,
    ...
    override fun onMessage(webSocket: WebSocket, text: String) {
        try {
            when (text) {
                "FORCE_EXPORT_INVOICE" -> {
                    //EXPORTAR FACTURAS.
                    GlobalScope.launch(Dispatchers.IO) {
                        val cantidadFacturas = countOinvPendientesUseCaseAlarmManager.CountPendientes()
                        if (cantidadFacturas > 0) {
                            val lotesPendientes =
                                getOinvPendientesExportarUseCaseAlarmManager.getPendientes()
                        ...
                        if (countNroFacturaPendienteUseCaseAlarmManager.Obtener() > 0) {
                            ...
                            exportarNroFacturaPendientesUseCaseAlarmManager.exportarDatos(
                                lotesPendientes
                            )
                        }
    ...
    fun connectToServer(url: String): WebSocket {
        val request = okhttp3.Request.Builder()
            ...
            ) // Agrega el ID del usuario como un encabezado personalizado
            .build()
    ...
    fun closeWebSocket() {
        webSocket?.close(
            1000,
            "Cierre normal"
        ) // Cierra el WebSocket con código 1000 y mensaje de cierre "Cierre normal"
